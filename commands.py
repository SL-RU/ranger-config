...
from compress import compress, extract_here
...
class fdsearch(Command):
    """
    :fdsearch [-d<depth>] <query>
    ...
    def execute(self):
        ...
        if self.arg(1):
            ...
            self.fm.notify(":fdsearch needs a query.", bad=True)
            return
...
class fdnext(Command):
    """
    :fdnext
    Selects the next match from the last :fdsearch.
    """

    def execute(self):
        if len(fdsearch.SEARCH_RESULTS) > 1:
            fdsearch.SEARCH_RESULTS.rotate(-1)  # rotate left
            self.fm.select_file(fdsearch.SEARCH_RESULTS[0])
        elif len(fdsearch.SEARCH_RESULTS) == 1:
            self.fm.select_file(fdsearch.SEARCH_RESULTS[0])


class fdprev(Command):
    """
    :fdprev
    Selects the next match from the last :fdsearch.
    """
    ...
    def execute(self):
        if len(fdsearch.SEARCH_RESULTS) > 1:
            fdsearch.SEARCH_RESULTS.rotate(1)  # rotate right
            self.fm.select_file(fdsearch.SEARCH_RESULTS[0])
        elif len(fdsearch.SEARCH_RESULTS) == 1:
            self.fm.select_file(fdsearch.SEARCH_RESULTS[0])
